...
namespace API.LABURNUM.COM.FrontEndApi
{
    public class AttendanceClassUKGApi
    {
        ...
        public void Update(DTO.LABURNUM.COM.AttendanceClassUKGModel model)
        {
            ...
            dbAttendanceClassUKG[0].IsPresentAfterLuch = model.IsPresentAfterLuch;
            dbAttendanceClassUKG[0].LunchAttendanceDate = new Component.Utility().GetISTDateTime();
            dbAttendanceClassUKG[0].LastUpdated = new Component.Utility().GetISTDateTime();
            this._laburnum.SaveChanges();
        }
        ...
        public List<API.LABURNUM.COM.AttendanceClassUKG> GetAttendanceClassUKGByAdvanceSearch(DTO.LABURNUM.COM.AttendanceClassUKGModel model)
        {
            ...
            else { if (model.IsPresentInMorning) { iQuery = this._laburnum.AttendanceClassUKGs.Where(x => x.IsPresentInMorning == model.IsPresentInMorning && x.IsActive == true); } }
            //Search By Present.
            ...
            else { if (model.IsPresentAfterLuch) { iQuery = this._laburnum.AttendanceClassUKGs.Where(x => x.IsPresentAfterLuch == model.IsPresentAfterLuch && x.IsActive == true); } }
            ...
            else
            {
                ...
                if (model.StartDate.Year != 0001)
                {
                    iQuery = this._laburnum.AttendanceClassUKGs.Where(x => x.CreatedOn >= model.StartDate && x.CreatedOn <= model.EndDate && x.IsActive == true);
                }
            }
