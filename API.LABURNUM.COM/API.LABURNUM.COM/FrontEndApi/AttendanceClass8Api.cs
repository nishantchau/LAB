...
namespace API.LABURNUM.COM.FrontEndApi
{
    public class AttendanceClass8Api
    {
        ...
        public void Update(DTO.LABURNUM.COM.AttendanceClass8Model model)
        {
            ...
            dbAttendanceClass8[0].IsPresentAfterLuch = model.IsPresentAfterLuch;
            dbAttendanceClass8[0].LunchAttendanceDate = new Component.Utility().GetISTDateTime();
            dbAttendanceClass8[0].LastUpdated = new Component.Utility().GetISTDateTime();
            this._laburnum.SaveChanges();
        }
        ...
        public List<API.LABURNUM.COM.AttendanceClass8> GetAttendanceClass8ByAdvanceSearch(DTO.LABURNUM.COM.AttendanceClass8Model model)
        {
            ...
            else { if (model.IsPresentInMorning) { iQuery = this._laburnum.AttendanceClass8.Where(x => x.IsPresentInMorning == model.IsPresentInMorning && x.IsActive == true); } }
            ...
            else { if (model.IsPresentAfterLuch) { iQuery = this._laburnum.AttendanceClass8.Where(x => x.IsPresentAfterLuch == model.IsPresentAfterLuch && x.IsActive == true); } }
            ...
            else
            {
                ...
                if (model.StartDate.Year != 0001)
                {
                    iQuery = this._laburnum.AttendanceClass8.Where(x => x.CreatedOn >= model.StartDate && x.CreatedOn <= model.EndDate && x.IsActive == true);
                }
            }
